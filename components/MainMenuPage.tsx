    ...
    return (
        <main className="flex-grow flex bg-gray-200">
            ...
            <div className="flex-grow p-4 flex">
                ...
                <section className="w-2/5 pl-4 flex flex-col">
                    <div className="bg-white p-6 rounded-lg shadow-md flex-grow">
                        <h2 className="text-xl font-bold text-center text-gray-800 mb-4">Agregar Artículos</h2>
                        <div className="text-center mb-6">
                            <span className="font-semibold text-gray-600">Muebles | Ropa</span>
                            <button disabled className="w-full mt-2 py-3 text-lg bg-gray-100 border-2 border-gray-300 rounded-lg text-gray-400 cursor-not-allowed">
                                Escanear SKU
                            </button>
                        </div>
                        <div>
                           <h3 className="text-lg font-semibold text-center text-gray-700 mb-4">Captura Manual</h3>
                           <div className="grid grid-cols-2 gap-x-8 gap-y-4 text-center justify-items-center">
                               <ActionButton icon={<FurnitureIcon className="w-8 h-8"/>} label="Muebles" disabled/>
                               <ActionButton icon={<ShirtIcon className="w-8 h-8"/>} label="Ropa" disabled/>
                               <ActionButton icon={<ShieldIcon className="w-8 h-8"/>} label="Seguros y Servicios" disabled/>
                               <ActionButton icon={<ShieldCheckIcon className="w-8 h-8"/>} label="Seguimiento de Facturas Motos" onClick={onNavigateToInvoices}/>
                           </div>
                        </div>
                    </div>
                    <div className="mt-4">
                       <button disabled className="w-full flex items-center justify-center gap-2 py-3 bg-blue-600 text-white font-bold rounded-lg shadow-md cursor-not-allowed">
...
const ActionButton: React.FC<{icon: React.ReactNode, label: string, disabled?: boolean, onClick?: () => void}> = ({ icon, label, disabled, onClick }) => (
    <div className="flex flex-col items-center">
        <button
            onClick={onClick}
            disabled={disabled}
            className="w-16 h-16 bg-white border-2 border-blue-500 text-blue-500 rounded-full flex items-center justify-center shadow-lg transition-transform transform enabled:hover:scale-105 disabled:opacity-60 disabled:cursor-not-allowed"
        >
            {icon}
        </button>
        <span className="mt-2 text-sm font-semibold text-gray-600 text-center">{label}</span>
    </div>
);
